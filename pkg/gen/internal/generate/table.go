...
func getTableComment(db *gorm.DB, schemaName, tableName string) string {

	table, err := getTableType(db, tableName)
	if err != nil || table == nil {
		var tableComment string
		err = db.Raw("SELECT TABLE_COMMENT FROM information_schema.tables where TABLE_SCHEMA=? and TABLE_NAME=?", schemaName, tableName).Scan(&tableComment).Error
		if err != nil {
			return ""
		}
		return tableComment
